...
/** Resolve paginator --_* length vars to px (supports px, rem, unitless → px). */
const parseCssLengthPx = (value, rootFontPx) => {
    const s = String(value ?? '').trim()
    if (!s || s === 'auto') return NaN
    const root =
        rootFontPx ??
        (parseFloat(globalThis.getComputedStyle(document.documentElement).fontSize) || 16)
    const lower = s.toLowerCase()
    if (lower.endsWith('rem')) return parseFloat(s) * root
    if (lower.endsWith('em')) return parseFloat(s) * root
    if (lower.endsWith('px')) return parseFloat(s)
...
export class Paginator extends HTMLElement {
    ...
    constructor() {
        ...
        #top::before, #top::after {
            content: '';
            grid-column: 1 / -1;
            background: var(--paginator-margin-bg, transparent);
            z-index: 1;
            pointer-events: none;
        }
        #top::before {
        ...
        #header, #footer {
            display: var(--paginator-marginal-display, grid);
            height: var(--_margin);
        }
        :is(#header, #footer) > * {
            display: flex;
    ...
    #beforeRender({ vertical, rtl, background }) {
        ...
        let maxInlineSize = parseCssLengthPx(style.getPropertyValue('--_max-inline-size'), rootPx)
        if (!Number.isFinite(maxInlineSize) || maxInlineSize <= 0) maxInlineSize = 720
        let maxColumnCount = parseInt(style.getPropertyValue('--_max-column-count-spread'), 10)
        if (!Number.isFinite(maxColumnCount) || maxColumnCount < 1) maxColumnCount = 2
        let margin = parseCssLengthPx(style.getPropertyValue('--_margin'), rootPx)
    ...
    isRectOnCurrentPage(rect) {
        // #element has `size` padding on each side of the iframe, so
        // visible content in iframe-space is [start - size, end - size]
        // — same formula used by #getVisibleRange (line 991)
        const visibleStart = this.start - this.size
        const visibleEnd = this.end - this.size
        return rect.right > visibleStart && rect.left < visibleEnd
    }
    scrollBy(dx, dy) {
    ...
    async #display(promise) {
        ...
        if (index == null || Number.isNaN(index)) {
            console.error('[foliate-paginator] #display: missing section index', { index, src })
            return
        }
        this.#index = index
        ...
        if (src) {
            ...
            console.error('[foliate-paginator] #display: no document URL (first load failed?)', {
                index,
            })
            return
        }
    ...
    async #goTo({ index, anchor, select}) {
        if (index === this.#index) await this.#display({ index, anchor, select })
            ...
            await this.#display(Promise.resolve(this.sections[index].load())
                ...
                .catch(e => {
                    console.error('[foliate-paginator] Section load failed', index, e)
                    return {}
                }))
        }
    }
    async goTo(target) {
        ...
        if (resolved == null || typeof resolved.index !== 'number') {
            console.error('[foliate-paginator] goTo: invalid resolved target', resolved)
            return
        }
        if (this.#canGoToIndex(resolved.index)) return this.#goTo(resolved)
        console.warn('[foliate-paginator] goTo: index out of range (no-op)', resolved.index)
    }
    #scrollPrev(distance) {
