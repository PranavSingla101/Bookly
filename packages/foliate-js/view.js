...
const fetchFile = async url => {
    const resolved =
        typeof url === 'string' && url.startsWith('http')
            ? url
            : new URL(url, globalThis.location?.href ?? 'https://invalid.invalid/').href
    console.log('[foliate] fetch EPUB:', resolved)
    const res = await fetch(resolved, { credentials: 'same-origin' })
    if (!res.ok) {
        console.error('[foliate] fetch EPUB failed:', res.status, res.statusText, resolved)
        throw new ResponseError(
            `${res.status} ${res.statusText}`, { cause: res })
    }
    const blob = await res.blob()
    console.log('[foliate] EPUB bytes:', blob.size)
    const path = (() => {
        try {
    ...
    return new File([blob], path || 'book.epub', { type: blob.type || 'application/epub+zip' })
}
...
export class View extends HTMLElement {
    ...
    constructor() {
        super()
        // Ensure the host passes a definite block size into shadow children
        // (foliate-paginator uses height: 100%). Light-DOM-only styling can be
        // unreliable when the element has no light children.
    ...
    goToTextStart() {
        const landmarkHref = this.book.landmarks
            ?.find(m => m.type.includes('bodymatter') || m.type.includes('text'))
            ?.href
        const firstLinearIndex = this.book.sections.findIndex(s => s.linear !== 'no')
        // findIndex returns -1 when every spine item is linear="no"; paginator.goTo
        // ignores invalid indices and leaves the viewport blank.
        const fallbackIndex = firstLinearIndex >= 0 ? firstLinearIndex : 0
        const target = landmarkHref ?? fallbackIndex
